...
-(void)getData:(void(^)(void))success failureBlock:(void(^)(void))failure{
    ...
    self.detailArr = @[@"特点: 在当前线程中执行任务，不会开启新线程，执行完一个任务，再执行下一个任务。",
                       ...
                       @"特点: 延迟几秒钟执行block里面的代码",
                       @"特点: 在项目中(只会执行一次) dispatch_once (可以使用单例)",
                       @"特点: (他的作用有点类似 for 循环) 在串行队列中  在异步或者并发队列中他的优势更能提现出来",
                       @"特点: dispatch_group_notify不会阻塞当前线程",
                       @"特点: dispatch_group_wait 会阻塞当前线程",
                       @"特点: 等到前面的异步操作执行完之后 回到主线程 可适用于 一个页面需要调用两个接口完成后在刷新页面的情况",
                       @"特点: 将异步执行任务转换为同步执行任务。",
                       @"特点: 在不考虑线程安全，不使用 semaphore 的情况下，得到票数是错乱的，这样显然不符合我们的需求，所以我们需要考虑线程安全问题",
                       @"特点: 可以解决多个线程同步的问题。"];
    if (self.dataArr.count != 0 && self.detailArr.count != 0){
        if (success) {
            success();
        }
    }else{
...
#pragma mark 任务+队列 相关方法 任务+队列 相关方法  都满足先进先出的原则*/
/*
 同步执行 + 并发队列
